...
2. 使用nginx进行反向代理，这样在输入域名的时候可以将默认到80端口的流量转发到博客平台使用的端口(等域名正式可以投入使用了，可以修改配置`/etc/nginx/nginx.conf`禁止外界直接访问博客服务的端口，只能经过nginx的转发)  <u>这样可以在nginx这边进行配置，对恶意流量进行拦截？不过直接在后端那里检测拦截原理应该是一样的，区别只是自己敲代码还是用人家开源的罢了
